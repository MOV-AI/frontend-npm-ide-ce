...
export default class BaseLink extends BaseLinkStruct {
  ...
  _renderPath = () => {
    ...
    this.object = d3
      .create("svg")
      .attr("id", `path-${this.canvas.canvasId}-${this.data.id}`)
      .attr("x", 0)
      .attr("y", 0)
    ...
    this.object
      .append("svg:defs")
      .append("filter")
      .attr("id", `link-shadow-${this.canvas.canvasId}-${this.data.id}`)
      .append("feDropShadow")
      .attr("dx", "1")
      .attr("dy", "1")
      .attr("stdDeviation", "1");
  ...
  _styleMouseOver = () => {
    this.path
      ...
      .attr("marker-mid", `url(#${this.canvas.canvasId}-markerselected)`);
  };
