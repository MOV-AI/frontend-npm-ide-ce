...
class Canvas {
  ...
  _initSvg = () => {
    const appTheme = this.mInterface.theme;
    console.log("debug canvas container id", this.container.current.id);
    this.svg = d3
      ...
      .attr(
        "class",
        `flow-interface flow-interface-${this.uid} ${appTheme.flowEditor.interfaceColor}`
      )
  ...
  reload = () => {
    ...
    this.svg.attr("class", `${appTheme.flowEditor.interfaceColor}`);
  };
